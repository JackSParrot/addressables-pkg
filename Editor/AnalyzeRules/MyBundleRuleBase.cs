...
namespace UnityEditor.AddressableAssets.Build.AnalyzeRules
{
    class MyBundleRuleBase : AnalyzeRule
    {
        ...
        internal void ConvertBundleNamesToGroupNames(AddressableAssetsBuildContext buildContext)
        {
            ...
            foreach (var assetGroup in buildContext.Settings.groups)
            {
                ...
                if (buildContext.assetGroupToBundles.TryGetValue(assetGroup, out bundles))
                {
                    ...
                    {
                        var keys = m_ExtractData.WriteData.FileToBundle.Keys.Where(key =>
                            m_ExtractData.WriteData.FileToBundle[key] == bundle);
        ...
        internal static List<AddressableAssetEntry> PrepGroupBundlePacking(AddressableAssetGroup assetGroup,
            List<AssetBundleBuild> bundleInputDefs, BundledAssetGroupSchema.BundlePackingMode packingMode)
        {
            ...
            switch (packingMode)
            {
                ...
                case BundledAssetGroupSchema.BundlePackingMode.PackSeparately:
                {
                    foreach (AddressableAssetEntry a in assetGroup.entries)
                    {
                        ...
                        a.GatherAllAssets(allEntries, true, true, false);
                        combinedEntries.AddRange(allEntries);
                        GenerateBuildInputDefinitions(allEntries, bundleInputDefs, CalculateGroupHash(allEntries),
                            a.address);
                    }
                }
                    break;
        ...
        internal static AssetBundleBuild GenerateBuildInputDefinition(List<AddressableAssetEntry> assets, string name)
        {
            ...
            assetsInputDef.addressableNames =
                new string[0]; //assets.Select(s => s.GetAssetLoadPath(true, assetInternalIds)).ToArray();
        ...
        internal List<AnalyzeResult> CalculateBuiltInResourceDependenciesToBundleDependecies(
            AddressableAssetSettings settings, string[] builtInResourcesPaths)
        {

...
class CheckBundleDupeDependenciesV2 : MyBundleRuleBase
{
    ...
    [SerializeField]
    internal Dictionary<List<string>, List<string>> duplicateAssetsAndParents =
    ...
    internal void BuildImplicitDuplicatedAssetsSet(IEnumerable<CheckDupeResult> checkDupeResults)
    {
        foreach (var checkDupeResult in checkDupeResults)
        {
            ...
            List<string> assets;
            if (!groupData.TryGetValue(m_ExtractData.WriteData.FileToBundle[checkDupeResult.DuplicatedFile],
                out assets))
            {
    ...
    public override void FixIssues(AddressableAssetSettings settings)
    {
        ...
        foreach (var entry in duplicateAssetsAndParents)
        {
            ...
            {
                entriesToAdd.Add(settings.CreateOrMoveEntry(AssetDatabase.AssetPathToGUID(assetPath).ToString(), group,
                    false, false));
            }
    ...
    internal void SetLabelValueForEntries(AddressableAssetSettings settings, List<AddressableAssetEntry> entries,
